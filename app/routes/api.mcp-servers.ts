...
async function handleGetRequest(request: Request, runtimeManager: MCPRuntimeManager) {
  ...
  switch (action) {
    ...
    case 'auto-discover': {
      ...
      const servers = runtimeManager.getAllServerStatus(true);
    ...
    case 'refresh': {
      // Refresh server status - handle both single server and all servers
      if (!serverId) {
        // If no serverId provided, refresh all servers
...
async function handlePostRequest(request: Request, runtimeManager: MCPRuntimeManager) {
  try {
    // Parse request body
    const body = (await request.json()) as AddServerRequest;
